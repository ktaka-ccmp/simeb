...
class Particle
{
 public:
  ...
  TXYZ    xyzPosition;
  TXYZ    xyzVelocity;
  TXYZ    xyzPositionCommit;
  TXYZ    xyzVelocityCommit;
...
{
  Fly(xyzPosition, xyzVelocity, rTf - rTi);

  xyzPosition += xyzPositionCommit;
  xyzVelocity += xyzVelocityCommit;
  xyzPositionCommit = xyzVelocityCommit = xyz0;
}
...
inline bool Particle::NextEvent(void) {
  ...
  while ((r_dz = Column::rGetZ(iPlane) - xyzPosition.rZ) < Column::rGetCapture())
...
inline bool Particle::Cross(real const r_dz)
{
  ...
  Fly(xyzPosition, xyzVelocity, r_dt);
...
inline real Particle::Impact(real const r_dz) {
  ...
  const real r_vz = xyzVelocity.rZ;
